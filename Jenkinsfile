        ...
        stage('SonarQube Analysis') {
            steps {
                script {
                    ...
                    withSonarQubeEnv() {
                        sh "${mvn}/bin/mvn clean verify sonar:sonar -Dsonar.host.url=http://${SONARQUBE_URL}:9000 -Dsonar.projectKey=Sonar-token -Dsonar.projectName=demo-app Maven Webapp"
                    }
                }
            }
        }
